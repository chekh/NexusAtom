...
#### 6. Неконсистентная доставка по типу цели
Наблюдаемое поведение в интерфейсе отправки:
- При выборе типа цели `Все` отправка не происходит.
- При выборе типа цели `Группа` и указании группы `all-users` сообщение доставляется всем.

❓ **Вопросы**:
- Должен ли `Все` быть алиасом для группы `all-users` на уровне backend?
- Почему маршрутизация для `Все` не попадает в общий пайплайн отправки?
- Где нормализуются значения типа цели (UI, API gateway, service layer)?

✅ **Ожидаемое поведение**:
- Тип цели `Все` отправляет сообщение всем получателям.
- Тип цели `Группа` + `all-users` эквивалентен `Все` и даёт тот же результат.
- При невозможности отправки пользователь получает явную ошибку, а не «тихий» no-op.
...
## Связанные артефакты
- [[ENG-027]] — вопрос о границах гибкости FSM
- [[ENG-031]] — отсутствие CLI/API для безопасного взаимодействия

---
*Вопрос зафиксирован в рамках критического аудита*
*Ссылка на родительский запрос: [[ENG-025]]*
